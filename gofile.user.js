...
(async function () {
  ...
  const I18N = {
    'zh-CN': {
      ...
      abdm_download_folder_placeholder: '默认为 D:/Downloads',
      abdm_port: 'ABDM 端口',
      ...
      aria2_rpc_dir_placeholder: '默认为 D:/Downloads',
    ...
    'en-US': {
      ...
      abdm_download_folder_placeholder: 'Default is D:/Downloads',
      abdm_port: 'ABDM Port',
      ...
      aria2_rpc_dir_placeholder: 'Default is D:/Downloads',
      ...
      please_make_sure_you_have_configured_download_folder:
        'Before downloading, make sure you have configured the download folder correctly.',
      reference: 'Reference',
  ...
  const ICONS = {
    download:
      '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-down-to-line-icon lucide-arrow-down-to-line size-5"><path d="M12 17V3"/><path d="m6 11 6 6 6-6"/><path d="M19 21H5"/></svg>',
    ...
    alert:
      '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="triangle-alert" class="lucide lucide-triangle-alert mt-0.5 size-5 shrink-0 text-amber-400" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"></path><path d="M12 9v4"></path><path d="M12 17h.01"></path></svg>',
  };
  ...
  const utils = {
    ...
    async collectAllItems(selectedKeys = []) {
      ...
      const collectItems = async (contentData, parentPath = '', inheritedSelected = false) => {
        ...
        for (const key of Object.keys(contentData.children || {})) {
          ...
          const isSelected = inheritedSelected || directlySelected;
          ...
          if (childItem.type === 'folder') {
            ...
            try {
              ...
              if (res?.data) {
                ...
                toast(`${utils.getTranslation('failed_to_fetch_folder_content')} ${childItem.name}`, { type: 'error' });
              }
            } catch (error) {
              toast(`${utils.getTranslation('failed_to_fetch_folder_content')} ${childItem.name} ${error}`, { type: 'error' });
            }
          }
        }
      };
    ...
    getDownloadSelectedButton() {
      ...
      const updateButtonVisibility = () => {
        const hasSelection =
          GE_FileManager.state.fileView || (!!GE_FileManager?.state?.selection && GE_FileManager.state.selection.size > 0);
              ...
              onClick: () => {
                popup.open({
                  ...
                  actions: [
                    { label: utils.getTranslation('cancel'), variant: 'ghost' },
                    {
                      label: utils.getTranslation('confirm'),
                      variant: 'primary',
                      onClick: (h) => {
    ...
    getActionButtons() {
      ...
      return [ge_logo, ge_title, spacer, utils.getDownloadSelectedButton(), utils.getSettingsButton()];
    },
